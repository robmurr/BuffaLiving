...
$stmt = $conn->prepare("SELECT id, name, photo FROM users WHERE auth_token = ?");
$stmt->bind_param('s', $auth_token);
$stmt->execute();
$stmt->bind_result($user_id, $name, $photo);
$stmt->fetch();
...
// Encode the photo in base64 if it exists
$photo_base64 = $photo ? base64_encode($photo) : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Respond with user info and apartments
